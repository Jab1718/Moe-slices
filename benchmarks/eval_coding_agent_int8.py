"""
Benchmark Suite: Code Generation & Coding Agent Capabilities for Qwen3.8-Flash Selective INT8 (37.24 GB).
Đo đạc chính xác 9 bài test Code & Coding Agent trên bản Selective INT8:
1. Python/BinarySearch
2. Python/FibonacciDP
3. Python/ValidParentheses
4. Rust/AsyncTokioChannel
5. CPP/ThreadSafeQueue
6. TypeScript/AsyncDebounce
7. Agent/GrepSearchCall
8. Agent/SelfCorrectionDebug
9. Agent/UnifiedDiffGeneration
"""
...
class SelectiveINT8Experts(nn.Module):
    """
    Selective INT8 Expert Compression Layer:
    - 128 FFN Experts are quantized into INT8 (range -127 to 127, scale = max / 127.0).
    - Preserves 100% BF16 precision for Router Gate & Attention Backbone.
    """
    def __init__(self, orig_experts: nn.Module):
        ...
        with torch.no_grad():
            ...
            scale_gu = w_gu.abs().amax(dim=-1, keepdim=True) / 127.0
            scale_gu = torch.clamp(scale_gu, min=1e-8)
            q_gu = torch.clamp(torch.round(w_gu / scale_gu), -128, 127).to(torch.int8)

            w_dp = orig_experts.down_proj.data.cpu().float()
            scale_dp = w_dp.abs().amax(dim=-1, keepdim=True) / 127.0
            scale_dp = torch.clamp(scale_dp, min=1e-8)
            q_dp = torch.clamp(torch.round(w_dp / scale_dp), -128, 127).to(torch.int8)
...
def run_benchmark():
    ...
    print("=" * 90)
    print(" 🚀 BENCHMARK: QWEN3.8-FLASH SELECTIVE INT8 (37.24 GB) - 9-TASK CODE & AGENT EVAL")
    print(f"    • Model Source : {model_path}")
    print(f"    • Target Engine: Router BF16 + Experts Selective INT8")
    print("=" * 90)
    ...
    print("[*] Áp dụng Selective INT8 Compression cho 128 Experts (Router & Attention ở chuẩn BF16)...")
    t_quant = time.perf_counter()
    for layer in model.model.layers:
        orig_exp = layer.mlp.experts
        layer.mlp.experts = SelectiveINT8Experts(orig_exp)
    torch.cuda.empty_cache()
    print(f"[✓] Selective INT8 Compression hoàn tất trong {time.perf_counter() - t_quant:.2f}s")
    ...
    print(f"[*] Tổng VRAM Tĩnh Phân bổ (INT8 Experts + BF16 Backbone): {total_vram_used:.2f} GB")
    ...
    # 3. Danh mục 9 bài Test Suite giống hệt INT4
    benchmark_tasks = [
        {
    ...
    for task in benchmark_tasks:
        ...
        ttft_estimate = (t_total / max(num_generated, 1)) * 1000
    ...
    print("=" * 90)
    print(f" 🏆 TỔNG KẾT BENCHMARK SELECTIVE INT8:")
    ...
    print(f"    • VRAM Footprint      : {total_vram_used:.2f} GB")
    print("=" * 90)

    # Lưu kết quả JSON
    with open("./benchmarks/int8_benchmark_results.json", "w", encoding="utf-8") as f:
        json.dump({
            "pass_rate": pass_rate,
            "passed": passed_tests,
            "total": total_tests,
    ...
    print("\n[✓] Đã lưu báo cáo chi tiết vào: ./benchmarks/int8_benchmark_results.json")
